...
;4.) Write code defines symbolic names for several string literals. Use each symbolic name in a variable definition.
	myMessage EQU 'Hello, World!'						;Declared myMessage as an EQU type so that it can be reassigned to a symbol.
	myVar db myMessage									;Declared myVar and assigned myMessage to it. 
;End 4.)
...
;2.) Write code that defines symbolic constants for all seven days of the week. Create an array variable that uses the symbols as initializers.
	Sun = 0												;This block is just giving number values to all 7 days of the week.
	Mon = 1
	Tue = 2
	Wed = 3
	Thu = 4
	...
	warray BYTE Sun, Mon, Tue, Wed, Thu, Fri, Sat		;This is the array that stores all of the values for the days of the week declared above.
;End 2.)
...
;Initialize each variable to a value that is consistent with its data type.
	...
	var1 BYTE 9h										;This block is just using all of the different data types for this chapter and giving them values.
	var2 SBYTE -20
	var3 WORD 800
	var4 SWORD -1600
